...
import { CreateRecordDto, FilterRecordsDto, UpdateRecordDto } from './dto/record.dto';
...
export class RecordsController {
    ...
    @Get()
    @Roles(Role.VIEWER)
    @ApiOperation({ summary: 'List financial records with filters & pagination (Viewer+)' })
    @ApiResponse({ status: HttpStatus.OK, description: 'Return list of records.' })
    ...
    @Patch(':id')
    @Roles(Role.ANALYST)
    @ApiOperation({ summary: 'Update a record — own records only unless Admin (Analyst+)' })
    @ApiResponse({ status: HttpStatus.OK, description: 'Record updated.' })
    @ApiResponse({ status: HttpStatus.NOT_FOUND, description: 'Record not found.' })
    @ApiResponse({ status: HttpStatus.FORBIDDEN, description: 'Cannot update other user\'s record.' })
    update(
    ...
    @ApiResponse({ status: HttpStatus.NO_CONTENT, description: 'Record deleted.' })
