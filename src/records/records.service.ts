...
import { CreateRecordDto, UpdateRecordDto, FilterRecordsDto } from './dto/record.dto';
...
export class RecordsService {
    ...
    async findAll(user: AuthUser, filters: FilterRecordsDto) {
        const { type, category, dateFrom, dateTo, search, minAmount, maxAmount, page = 1, limit = 20 } = filters;
    ...
    async create(user: AuthUser, dto: CreateRecordDto) {
        ...
        await this.logAction('CREATE', 'FinancialRecord', record.id, user.id, JSON.stringify(dto));
        return record;
    }
    ...
    async update(user: AuthUser, id: string, dto: UpdateRecordDto) {
        ...
        await this.logAction('UPDATE', 'FinancialRecord', id, user.id, JSON.stringify(dto));
        return updated;
    }
    ...
    private async logAction(action: string, entity: string, entityId: string | null, userId: string, details?: string) {
        await this.prisma.systemLog.create({
            data: { action, entity, entityId, userId, details },
        }).catch((err: unknown) => console.error('Failed to create audit log', err));
    }
