...
describe('BudgetsService', () => {
    ...
    describe('findAll', () => {
        it('returns user budgets', async () => {
            mockPrisma.budget.findMany.mockResolvedValue([{ id: 'b1', category: 'Food' }]);
            const result = await service.findAll(mockUser);
            expect(result).toHaveLength(1);
            expect(mockPrisma.budget.findMany).toHaveBeenCalledWith(expect.objectContaining({
    ...
    describe('create', () => {
        it('upserts budget for user category', async () => {
            const dto = { category: 'Food', amount: 500, period: BudgetPeriod.MONTHLY };
    ...
    describe('remove', () => {
        it('deletes budget if owned by user', async () => {
            mockPrisma.budget.findUnique.mockResolvedValue({ id: 'b1', userId: 'user1' });
        ...
        it('throws NotFoundException if not owned', async () => {
            mockPrisma.budget.findUnique.mockResolvedValue({ id: 'b1', userId: 'other' });
            await expect(service.remove(mockUser, 'b1')).rejects.toThrow(NotFoundException);
        });
    });
});
